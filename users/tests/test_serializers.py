...
class UserSerializerTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="test@example.com", password="testpass123", phone_number="+1234567890", city="Test City"
        )
...
class TelegramLinkCreateSerializerTest(TestCase):
    ...
    def test_create_telegram_link(self):
        serializer = TelegramLinkCreateSerializer(context={"request": self.request})
    ...
    def test_expire_previous_links(self):
        old_link = TelegramLink.objects.create(
            user=self.user, code="OLD123456", expires_at=timezone.now() + timedelta(hours=1)
        )
...
class TelegramConfirmSerializerTest(TestCase):
    def setUp(self):
        ...
        self.valid_link = TelegramLink.objects.create(
            user=self.user, code="VALID123456", expires_at=timezone.now() + timedelta(minutes=15)
        )

    def test_confirm_valid_link(self):
        data = {"code": "VALID123456", "chat_id": 123456789}
    ...
    def test_confirm_nonexistent_code(self):
        data = {"code": "NONEXISTENT", "chat_id": 123456789}
    ...
    def test_chat_id_validation(self):
        data = {"code": "VALID123456", "chat_id": 0}
    ...
    def test_code_max_length(self):
        data = {"code": "A" * 33, "chat_id": 123456789}
